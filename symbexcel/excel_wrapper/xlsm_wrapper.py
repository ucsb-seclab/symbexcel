...
from symbexcel.boundsheet import *
from symbexcel.excel_wrapper import ExcelWrapper, RowAttribute

log = logging.getLogger(__name__)

class XLSMWrapper(ExcelWrapper):
    def __init__(self, xlsm_doc_path):
        self.xls_workbook = xlrd2.open_workbook(xlsm_doc_path)
        ...
        super().__init__(xlsm_doc_path)

    def __getstate__(self):
        state = dict(self.__dict__)
        del state['_workbook_relationships']
        del state['_workbook']
        del state['_workbook_rels']
        del state['_content_types']
        return state
    ...
    def load_names(self):
        ...
        if hasattr(workbook_obj.workbook, 'definedNames'):
            for defined_name in workbook_obj.workbook.definedNames.definedName:
                name = defined_name.get_attribute('name').replace('_xlnm.', '').lower()
                data = defined_name.cdata

                # try to parse a cell address
                sheet, col, row = Cell.parse_cell_addr(data)

                if sheet is None or col is None or row is None:
                    self._defined_names[name] = data
                else:
                    cell = self.get_cell(sheet, col, row)
                    if cell:
                        self._defined_names[name] = cell
                    else:
                        log.warning(f"Can't parse oREF defined name {data}")
    ...
    def load_macrosheets(self):
        ...
        for macrosheet in macrosheets:
            ...
            if hasattr(macrosheet['sheet_xml'].xm_macrosheet, 'sheetFormatPr'):
                macrosheet['sheet'].default_height = macrosheet[
                    'sheet_xml'].xm_macrosheet.sheetFormatPr.get_attribute('defaultRowHeight')
